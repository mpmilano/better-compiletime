...
namespace compile_time {
    ...
    namespace value_to_type {
        ...
        template<typename FValue, typename ctcx, typename T, std::size_t... indexes> constexpr auto convert_to_type_instance(const typename ctcx::Allocator &debug_allocator, std::integer_sequence<std::size_t,indexes...>){
        ...
        template<typename FValue, typename Allocator_holder, typename T> constexpr auto convert_to_type_f(const typename compile_time_context<Allocator_holder>::Allocator &debug_allocator, instance<T> const * const inst){
            (void) inst;
            using ctctx = compile_time_context<Allocator_holder>;
            return convert_to_type_instance<FValue,ctctx,T>(debug_allocator,std::make_index_sequence<struct_size<T>>{});
        }

        template<typename FValue, typename ctcx, std::size_t size, typename top, typename spec1, typename... specs> 
        constexpr auto convert_to_type_erased_ref(const typename ctcx::Allocator &debug_allocator, compile_time::Allocator<size,top,spec1,specs...>* = nullptr){
            if constexpr (FValue{}()){
                constexpr auto &ptr = FValue::value;
                if constexpr(ptr.template is_this_type<spec1>(ctcx::allocator)){
                    auto &as_single_allocator = ctcx::allocator.template as_single_allocator<spec1>();
                    auto &value = FValue::value;
                    auto &gotten = value.get(as_single_allocator);
                    (void) gotten; (void) value; (void) as_single_allocator;
                    struct_wrap(converted,FValue::value.get(ctcx::allocator.template as_single_allocator<spec1>()));
                    auto converted_value = converted{}(); (void) converted_value;
                    using wrapped = simple_wrapper<converted>;
                    auto wrapped_value = wrapped::value; (void) wrapped_value;
                    return ctcx::template convert_to_type_f<wrapped>();
                }
                else return convert_to_type_erased_ref<FValue, ctcx, size, top, specs...>(debug_allocator);
        ...
        template<typename FValue, typename Allocator_holder> constexpr auto convert_to_type_f(const typename compile_time_context<Allocator_holder>::Allocator &debug_allocator, erased_ref const * const){
            using ctcx = compile_time_context<Allocator_holder>;
            return convert_to_type_erased_ref<FValue,ctcx>(debug_allocator, (typename ctcx::Allocator*) nullptr);
        }

        template<typename FValue, typename Allocator_holder, typename T> constexpr auto convert_to_type_f(const typename compile_time_context<Allocator_holder>::Allocator &debug_allocator, allocated_ref<T> const * const){
            using ctcx = compile_time_context<Allocator_holder>;
    ...
    namespace ctctx {
    template<typename Allocator_holder> template<typename FValue>
    constexpr auto i<Allocator_holder>::convert_to_type_f(){
        ...
        else return value_to_type::convert_to_type_f<FValue, Allocator_holder>(allocator_ref, &value);
    }
    }
}