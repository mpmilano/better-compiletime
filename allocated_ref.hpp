...
public:
  ...
  template <std::size_t s> constexpr allocated_ref(SingleAllocator<s, T> &);

  template <std::size_t s> constexpr void free(SingleAllocator<s, T> &);
...
public:
  ...
  constexpr bool is_this_type(const SingleAllocator<size, T> &,
                              const Allocator<size, Top, Subs...> &a) const {
    return a.template get_allocator_index<SingleAllocator<size, T>>() ==
           allocator_index;
  }
